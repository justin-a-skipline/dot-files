...
#define GRAPHPRINTF1(value) do { \
	char text[150]; \
	int count = snprintf(text, sizeof(text), "\r\rRTGRAPH add " #value " %f", (float)(value)); \
	if (count > 0) write(9, text, (size_t)count); \
	} while (0)

#define GRAPHPRINTF2(value, x_value) do { \
	char text[150]; \
	int count = snprintf(text, sizeof(text), "\r\rRTGRAPH add " #value " %f %f", (float)(value), (float)(x_value)); \
	if (count > 0) write(9, text, (size_t)count); \
	} while (0)
